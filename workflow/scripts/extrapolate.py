...
def extrapolate(stats_path: str, extra_path: str, bootstrap_path: str, repeats: int):
    ...
    if (
        os.stat(stats_path).st_size == 0
    ):  # if the result is empty, could be caued by insufficient sample size
        Path(
            extra_path
        ).touch()  # for the snakemake workflow to know the process is finished
    ...
    ###
    metric = (
        "r2_test" if "r2_test" in df.columns else "acc_test"
    )  # decide which metrics based on classification or regression
    ...
    for _ in range(repeats):
        ...
        for i, n in enumerate(x):  # for each sample size
            y_bs_sample = df[df["n"] == n][metric].sample(
                frac=1, replace=True
        ...
        result.update(
            {
                ...
                "p_bootstrap_025": np.percentile(
                    p_bootstrap, 2.5, axis=0
                ),  # 95% confidence interval with the 2.5 cut off points
            }
        )
