...
def splitgen(
    ...
    splits = split_grid(
        y, n_samples=samples, n_seeds=n_seeds, n_val=n_val, n_test=n_test
    )
...
def main() -> None:
    ...
    splitgen_parser.add_argument(
        "--val", type=int, help="validation set size", default=1000
    )
    ...
    if args.run:
        ...
        splitgen(
            Path(args.data), args.label, args.seeds, args.samples, args.val, args.test
        )
    elif args.retrieve:
